...
import { InputGroup, InputLeftElement, Input, Icon, IconButton } from "@chakra-ui/react";
...
function Navbar({filterLabel, setFilterLabel}) {
    ...
    <NavBar>
      ...
      <Modal isOpen={filterIsOpen} onClose={filterOnClose}>
        ...
        <ModalContent width={"fit-content"}>
          <FilterModal filterLabel={filterLabel} close={filterOnClose} setFilterLabel={setFilterLabel} />
        </ModalContent>
      </Modal>

      {/* <Modal isOpen={handleInputFocus} onClose={searchOnClose}>
        <ModalOverlay />
        <ModalContent>
          <SearchResultModal />
          <Button onClick={searchOnClose}>Close</Button>
        </ModalContent>
        <ModalFooter>
        </ModalFooter>
      </Modal> */}
      ...
      <Right>
        <IconButton onClick={filterOnOpen} icon={<Icon boxSize="8" viewBox='0 0 24 24' fill='none' color='black' strokeWidth="1.5" stroke="currentColor">
